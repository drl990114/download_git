...
fn test_parse_shortcut_url() {
    ...
    let git_url = git_download::git_url_parser::parse_shortcut_url(url).unwrap();
...
fn test_parse_https_url() {
    ...
    let git_url = git_download::git_url_parser::parse_http_url(url).unwrap();
...
fn test_parse_url() {
    let url = "gh:twbs/bootstrap.git:master";
    let git_url = git_download::git_url_parser::parse_url(url).unwrap();
    assert_eq!(git_url.url, "https://github.com/twbs/bootstrap");
    assert_eq!(git_url.name, "bootstrap");
    assert_eq!(git_url.branch, "master");
}
